...
if (mb_strlen($correo, "UTF-8") > $tamCorreo) {
    print "  <h4 style='padding-left: 10px'>El correo no puede tener más de $tamCorreo caracteres.</h4>\n\n";
} else {
    $correoOk = true;
}
...
if ($nombreOk && $apellidosOk && $telefonoOk && $correoOk && $direccionOk) {
    $consulta = "UPDATE $dbTablaAgenda
        SET nombre=:nombre, apellidos=:apellidos, telefono=:telefono, correo=:correo, direccion=:direccion
        WHERE id=:id";
    $result = $db->prepare($consulta);
    if ($result->execute(array(":id" => $id, ":nombre" => $nombre, ":apellidos" => $apellidos
            , ":telefono" => $telefono, ":correo" => $correo, ":direccion" => $direccion))) {
        print "  <h4 style='padding-left: 10px'>Registro modificado correctamente.</h4>\n";
    } else {
        print "  <h4 style='padding-left: 10px'>Error al modificar el registro.</h4>\n";
    }
} else {
    print " <div style='text-align: center'><a href='modificar2.php?id=$id' class='btn btn-primary'>Volver</a>";
}
